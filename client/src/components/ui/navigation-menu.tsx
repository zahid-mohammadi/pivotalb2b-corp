...
const navigationMenuTriggerStyle = cva(
  "group relative inline-flex h-10 w-max items-center justify-center px-4 py-2 text-sm font-medium transition-all duration-300 hover:text-primary focus:outline-none disabled:pointer-events-none disabled:opacity-50 data-[active]:text-primary data-[state=open]:text-primary after:absolute after:bottom-0 after:left-0 after:h-[2px] after:w-0 after:bg-primary after:transition-all after:duration-300 hover:after:w-full data-[state=open]:after:w-full"
)
...
>(({ className, ...props }, ref) => (
  <NavigationMenuPrimitive.Content
    ref={ref}
    className={cn(
      "left-0 top-0 w-full bg-background/98 backdrop-blur-md data-[motion^=from-]:animate-in data-[motion^=to-]:animate-out data-[motion^=from-]:fade-in data-[motion^=to-]:fade-out data-[motion=from-end]:slide-in-from-right-52 data-[motion=from-start]:slide-in-from-left-52 data-[motion=to-end]:slide-out-to-right-52 data-[motion=to-start]:slide-out-to-left-52 md:absolute md:w-auto dark:border-gray-800 rounded-lg border shadow-lg",
      className
    )}
    {...props}
  />
...
>(({ className, ...props }, ref) => (
  <NavigationMenuPrimitive.Indicator
    ...
    {...props}
  >
    <div className="relative top-[60%] h-2.5 w-2.5 rotate-45 rounded-tl-sm bg-primary/20 shadow-md" />
  </NavigationMenuPrimitive.Indicator>
))
